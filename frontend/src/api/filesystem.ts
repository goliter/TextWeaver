...
export const filesystemApi = {
  getFileTree: async (flowId: number): Promise<FileWithChildren[]> => {
    const response = await api.get<FileWithChildren[]>(
      `/flows/${flowId}/files/tree`,
    );
  ...
  getFiles: async (
    ...
    const response = await api.get<FileBaseResponse[]>(
      `/flows/${flowId}/files`,
      {
        params:
  ...
  getAllFiles: async (flowId: number): Promise<FileBaseResponse[]> => {
    const response = await api.get<FileBaseResponse[]>(
      `/flows/${flowId}/files/all`,
    );
    return response.data;
  },
